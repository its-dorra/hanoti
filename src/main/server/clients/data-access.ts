...
export class ClientsDataAccess {
  ...
  async findAll(query: string, cursor?: ClientCursor | null, limit = 20) {
    const cursorDate = cursor ? cursor.createdAt : undefined
        ...
        and(
          ...
          cursor
            ? or(
                ...
                and(eq(clients.createdAt, cursorDate!), lt(clients.id, cursor.id))
              )
            : undefined
        )
      )
  ...
  async adjustDebt(tx: AppTransaction, clientId: number, delta: number) {
    const [row] = await tx
      .update(clients)
      .set({
        balance: sql<number>`${clients.balance} + ${delta}`,
        updatedAt: new Date()
      })
