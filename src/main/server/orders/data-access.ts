import { and, desc, eq, gte, lte, or, lt, like } from 'drizzle-orm'
...
export class OrdersDataAccess {
  ...
  async findByIdWithoutItems(id: number) {
    const [order] = await this.db
      .select({ ...ORDER_COLUMNS, clientName: clients.name })
      .from(orders)
      .where(eq(orders.id, id))
  ...
  async insertOrderItems(tx: AppTransaction, orderId: number, items: NewOrderItemRow[]) {
    return tx
      .insert(orderItems)
  ...
  async deleteOrder(tx: AppTransaction, orderId: number) {
